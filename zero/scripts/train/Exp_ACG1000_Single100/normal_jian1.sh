...
for LETTER in $(echo "$INPUT_TASK_LETTERS" | sed -e 's/\(.\)/\1 /g'); do
    ...
    python trainer_pl_all.py --config-name=DP_DecoupleActionHead_normal n_demo=100 task_name="$DESCRIPTIVE_TASK_NAME" task.env_runner.n_envs=28 task_alphabet=$LETTER training.val_every=1 logging.group=normal_100
