...
for LETTER in $(echo "$INPUT_TASK_LETTERS" | sed -e 's/\(.\)/\1 /g'); do
    ...
    python trainer_pl_all.py --config-name=DP_DecoupleActionHead_stage2_ABC_D n_demo=1000 task_name="$DESCRIPTIVE_TASK_NAME" task.env_runner.n_envs=28 task_alphabet=$LETTER dataloader.num_workers=8
